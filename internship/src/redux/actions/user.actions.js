...
import {
  getData,
  addEducation as Edu,
  addWorkExperience as Exp,
  addNewRecruiter as Rec,
  addSkills as skillTypes,
  addProfile as profileTypes,
} from "../actionTypes";
import axios from "../helper_axios";

export const getUserData = (testString) => {
  ...
  return async (dispatch) => {
    ...
    try {
      const res = await axios.post(
        "/internship/v1/company-recruiters/",
        main_obj
      );
      if (res.status === 201) {
...
export const addProfile = (
  ...
  try {
    const res = await axios.post("/skilzen/v1/profile/update_profile/", {
